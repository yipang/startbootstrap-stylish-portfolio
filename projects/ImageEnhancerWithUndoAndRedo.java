 ...
 * by Yi Pang for CSE 373 Assignment 1, Autumn, 2016.
 * Section BB.
 * This class will set different conditions for a picture.
 * With the function of REDO and UNDO to change to the previous mode.
 *
 * ImageEnhancer.java
...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	//Check different actions and prepare for the undo
    	if (e.getSource() != exitItem && e.getSource() != redoItem && e.getSource() != undoItem) {
    		BufferedImage curImage = biFiltered;
    	...
    	//When undo prepare for the future redo
    	if (e.getSource() == undoItem) {
    		BufferedImage cur = undoStack.pop();
    		redoStack.push(cur);
    		...
    		if (undoStack.isEmpty()) {
    			...
    			biFiltered = undoStack.get(undoStack.getSize() - 1);
    		}
    	}
    	
    	if (e.getSource() == redoItem) {
    		BufferedImage cur = redoStack.pop();
    		undoStack.push(cur);
    		biFiltered = undoStack.get(undoStack.getSize() - 1);
    		undoItem.setEnabled(true);
    ...
    public void run() {
        JFrame f = new JFrame("Image Enhancer WITH Undo AND Redo by Yi Pang"); // Students should update this.
